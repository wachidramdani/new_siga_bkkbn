...
class Kabupaten extends Component {	
    ...
    CellFormatter(cell, row) {
        return (<div><Link to={`/table15A/detailKabupaten?kabupaten=${row.provinsi}`}>{row.provinsi}</Link></div>);
    }
    ...
    render() {
        ...
        return (
            ...
            title = {"Tempat Pelayanan KB Berdasarkan Jenis Di Wilayah Miskin"}
            namaLaporan = {`Tempat Pelayanan KB Berdasarkan Jenis Di Wilayah Miskin di ${params.kabupaten}`}
            marginLeft = {260}
            header = {
                [
                    [
